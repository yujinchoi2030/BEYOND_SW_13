...
-- RAND 함수
-- 1~100 사시의 랜덤 값 출력
SELECT RAND() -- 0.0 ~ 0.999999999999...
		,RAND() * 100 -- 0.0 ~ 0.999999999999...
		,FLOOR(RAND() * 100);

-- 날짜 및 시간 함수
-- ADDDATE, SUBDATE 함수
SELECT ADDDATE('2025-01-01', INTERVAL 10 DAY), ADDDATE('2025-01-01', INTERVAL 1 MONTH), ADDDATE('2025-01-01', INTERVAL 2 YEAR);

-- employee 테이블에서 직원명, 입사일, 입사 후 3개월이 된 날짜 조회
SELECT EMP_NAME, HIRE_DATE, ADDDATE(HIRE_DATE, INTERVAL 3 MONTH) FROM employee;

-- ADDTIME, SUBTIME 함수
SELECT ADDTIME('2024-12-17 11:28:30', '1:1:1'), ADDTIME('11:28:30', '2:2:2');

-- CURDATE, CURTIME, NOW, SYSDATE 함수
SELECT CURDATE(), CURTIME(), NOW(), NOW();

-- YEAR, MONTH, DAYOFMONTH, DATE 함수
SELECT YEAR(CURDATE()), MONTH(CURDATE()),DAYOFMONTH(CURDATE()),DATE(NOW());

-- HOUR, MINUTE, SECOND, MICROSECOND, TIME 함수
SELECT HOUR(CURDATE()), MINUTE(CURDATE()),SECOND(CURDATE()),MICROSECOND(CURDATE()),TIME(NOW());

-- DATEDIFF, TIMEDIFF 함수
-- DATEDIFF, TIMEDIFF 함수
SELECT DATEDIFF(CURDATE(), '2024-05-25'), TIMEDIFF(CURTIME(), '09:00:00');

-- employee 테이블에서 직원명, 입사일, 근무 일 수 조회
SELECT emp_name,hire_date,DATEDIFF(CURDATE(), hire_date) FROM employee;

-- DAYOFWEEK, MONTHNAME, DAYOFYEAR, LAST_DAY 함수
SELECT DAYOFWEEK(CURDATE()), MONTHNAME(CURDATE()), DAYOFYEAR(CURDATE()), LAST_DAY(CURDATE());

-- MAKEDATE, MAKETIME, PERIOD_ADD, PERIOD_DIFF 함수
SELECT MAKEDATE(2025, 100),MAKETIME(22, 58, 59),PERIOD_ADD(202505, 11),PERIOD_DIFF(202405, 202505);

-- QUARTER, TIME_TO_SEC 함수
SELECT QUARTER(CURDATE()), TIME_TO_SEC('00:01:30'), TIME_TO_SEC(CURTIME());
